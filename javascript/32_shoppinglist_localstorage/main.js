...
addgiftObj.onclick = function () {
    if (giftObj.value !== ``) {
        ...
        localStorage.setItem(`Warenkorb`, JSON.stringify(basket));
        console.log(localStorage.Warenkorb);
    } else {
...
function start() {
    basket = JSON.parse(localStorage.getItem(`Warenkorb`));
    console.log(basket);
    for (let i = 0; i < basket.length; i++) {
        const newLiObj = document.createElement('li');
...
// 1. Wenn der Nutzer eine neues Item hinzufügt, dann darf die
//    Textbox nicht leer sein.
//    Gib also eine Fehlermeldung aus.

// 2. analog zu 1. für Das Entfernen.

// 3. wenn der Nutzer die Seite neulädt, sollen alle ausgewählten Items nicht 
//    gelöscht werden, sondern weiterhin in der Liste bestehen